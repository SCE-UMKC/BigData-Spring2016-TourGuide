...
object IPApp {
  ...
  def generateRandomForestModel(sc: SparkContext): Unit = {
    ...
    parsedData.foreach(f=>println("F: " + f))
    ...
    println("numOfTrees string: " + numOfTrees.toString())
    println("Size of numOfTrees: " + numOfTrees.size)
      ...
      strategies.foreach(featureSubsetStrategy => {
        impurities.foreach(impurity => {
          maxDepths.foreach(maxDepth => {
            ...
            println("training: " + training.toString() + "; numClasses: " + numClasses + "; categoricalFeaturesInfo: " + categoricalFeaturesInfo.toString() + "; maxBins: " + maxBins)
  ...
  def testImageClassification(sc: SparkContext, path: String) = {
    ...
    println("vocabulary: " + vocabulary.toString)
   ...
   // val path = "files/101_ObjectCategories/ant/image_0012.jpg"
    ...
    println("desc: " + desc.toString)

    val testImageMat = imread(path)
    println("testImageMat: " + testImageMat.toString)
    //imshow("Test Image", testImageMat)

    val histogram = ImageUtils.matToVector(desc)
    println("histogram: " + histogram.toString)
    ...
    println("nbModel: " + nbModel.toString())

    val p = nbModel.predict(histogram)
    println(s"Predicting test image " + path + " as " + IMAGE_CATEGORIES(p.toInt))
  ...
  def main(args: Array[String]) {
    ...
    val testData = new File("sign_data/test").listFiles.filter(_.isFile)
    testData.foreach(f => {
      println("F is: " + f.toString)
      val classf = testImageClassification(sc, f.toString)
      results += f.toString -> classf
    })
    //testImageClassification(sc, "sign_data/train/Exit_Translucent/Exit_020.jpg")
    //testImageClassification(sc, "sign_data/train/Stop/stop11.png")

    println("size of results: " + results.size)
    results.foreach(f => {
      println("results are: " + f._1 + " = " + f._2)
    })
  }
}
